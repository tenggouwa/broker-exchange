          ...
          <div class="w-box m-l">
            <div>
              <div class="content" >
                ...
                <scrollCom :proessnum="sellScroll" bgclass="range_sell" showcoin="USDT" :showcionType="getsymbol(symbol)" :coastnum="sellmoney" :nextnum="nextcoin" @proessClick="canChange('SELL')" @change="sellscroll"></scrollCom>
              ...
              <div class="content">
                <p>
                  <span class="spanl" @click="buyAllNum">可用(USDT) :<b class="availabel">{{myAssets.availablemoney}}</b></span>
                </p>
                <p class="djies">
                  <span class="spanl">冻结(USDT) :<b>{{myAssets.frozenmoney}}</b>  </span>
                </p>
                <div class="row-control">
                ...
                <scrollCom :proessnum="buynumm" bgclass="range_buy" showcoin="USDT" showcionType="USDT" :coastnum="buymoney" :nextnum="nextMoney" @proessClick="canChange('BUY')" @change="buyScroll"></scrollCom>
                <div class="row-control fn-clear">
          ...
          <div class="w-box m-l">
            ...
            <table class="market">
              ...
              <tr v-for="(item, index) in buyticket">
                ...
                <td class="last" @click="maichuNum(index)">{{ item.am}}</td> <!-- |formatNum -->
              </tr>
            </table>
          </div>
          <div class="w-box m-r">
...
import { ajax,getticketcom,setcookie,getcookie,delatecookie,gettxfees,formatcoinPrice,formatcoinNum,getmincoinNum,formatcoinprice,initINput,numjs,nitFince} from '../../assets/js/common'
...
export default {
  ...
  methods:{
    ...
    inputTradeNum(num,type){
      ...
      inputTradeNumCom(num,type,this,'USDT');
    },
    ...
    mairuNum(numIndex){
      ...
      var buymoneyes=parseFloat(this.buyprice*this.buynum).toFixed(4);//USDT 4位
    ...
    maichuNum(numIndex){
     ...
     if(Number(formatcoinNum(this.symbol))==0){
      ...
      if(this.myAssets.availablecoin==0){
        ...
        this.coastcoin=parseFloat(this.sellprice*this.sellnum).toFixed(4);
      }
      this.sellmoney=parseFloat(this.sellprice*this.sellnum*0.999).toFixed(4);
    ...
    sletprice(tradetype,index){//价格框
      ...
      this.sellmoney=parseFloat(this.sellprice*this.sellnum*0.999).toFixed(4);
       this.buymoney=parseFloat(this.buyprice*this.buynum).toFixed(4);
      this.changMaxNum();
    },
    flashRoute: function() {
      ...
      this.commomstart();
      switch(path){
        case '/usercenter/bccUSDTtrade':
        break;
        case '/usercenter/ethUSDTtrade':
        break;
        case '/usercenter/etcUSDTtrade':
        break;
        case '/usercenter/zecUSDTtrade':
        break;
        case '/usercenter/ltcUSDTtrade':
        break;
        case '/usercenter/btcUSDTtrade':
        break;
        case '/usercenter/btyUSDTtrade':
        break;
        case '/usercenter/yccUSDTtrade':
        break;
        case '/usercenter/btsUSDTtrade':
        break;
        case '/usercenter/scUSDTtrade':
        break;
        case '/usercenter/btcsUSDTtrade':
        break;
      }
    },
    //获取用户资金
    getusermoney(){
      ...
      ajax(this,this.extendApi.getMoneyInfo,'',function(reponse){
        if(reponse.code==200){
          ...
          if(_this.buynumm==0){
            _this.nextMoney=data.list.USDT.active;
          }
    ...
    buyScroll(val){
      buyscrolles(val,this,'USDT');
    },
    sellscroll(val){
      sellscrolles(val,this,'USDT');
    },
    //交易函数
    tradeC(tradetype,price){
      ...
      this.tradeparam.currency=sessionStorage.getItem('symbol').substr(0,sessionStorage.getItem('symbol').length-4);
      //加上价格振幅10%的提示
        ...
        if(response.code == 200){
          if(_this.tradeparam.ty=='BUY'){
           ...
           let txfee=0;
            ...
            _this.nextMoney=(Number(_this.myAssets.availablemoney)-0.01)/((1+Number(txfee))*_this.tradeparam.price);
            // _this.nextMoney=(_this.myAssets.availablemoney-(_this.myAssets.availablemoney*txfee))/_this.tradeparam.price;//买入情况下剩余的钱
            // _this.nextMoney=_this.myAssets.availablemoney;//买入情况下剩余的钱
          }else{
            _this.sellScroll=0;
